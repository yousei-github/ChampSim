...
int MEMORY_CONTROLLER::add_rq(PACKET* packet)
{
  ...
  MemoryType type = get_memory_type(packet->address);

  switch (type)
  {
  case HBM:
  {
    HBM_CHANNEL& channel = hbm_channels[hbm_get_channel(packet->address)];
    ...
    {
      // find the address in write queue to read.
    ...
    {
      // find the address in read queue to merge.
  ...
  case DDR:
  {
    DDR_CHANNEL& channel = ddr_channels[ddr_get_channel(packet->address)];
...
uint32_t MEMORY_CONTROLLER::memory_get_channel(uint64_t address)
{
  ...
  {
  case HBM:
    channel = hbm_get_channel(address);
    break;
  case DDR:
    channel = ddr_get_channel(address);
    break;
  }
...
uint32_t MEMORY_CONTROLLER::memory_get_bank(uint64_t address)
{
  ...
  {
  case HBM:
    bank = hbm_get_bank(address);
    break;
  case DDR:
    bank = ddr_get_bank(address);
    break;
  }
...
uint32_t MEMORY_CONTROLLER::memory_get_column(uint64_t address)
{
  ...
  {
  case HBM:
    column = hbm_get_column(address);
    break;
  case DDR:
    column = ddr_get_column(address);
    break;
  }
...
uint32_t MEMORY_CONTROLLER::memory_get_row(uint64_t address)
{
  ...
  {
  case HBM:
    row = hbm_get_row(address);
    break;
  case DDR:
    row = ddr_get_row(address);
    break;
  }
...
uint32_t MEMORY_CONTROLLER::hbm_get_channel(uint64_t address)
{
  int shift = LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(HBM_CHANNELS));
}

uint32_t MEMORY_CONTROLLER::hbm_get_bank(uint64_t address)
{
  int shift = lg2(HBM_CHANNELS) + LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(HBM_BANKS));
}

uint32_t MEMORY_CONTROLLER::hbm_get_column(uint64_t address)
{
  int shift = lg2(HBM_BANKS) + lg2(HBM_CHANNELS) + LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(HBM_COLUMNS));
}

uint32_t MEMORY_CONTROLLER::hbm_get_row(uint64_t address)
{
  int shift = lg2(HBM_BANKS) + lg2(HBM_COLUMNS) + lg2(HBM_CHANNELS) + LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(HBM_ROWS));
}

uint32_t MEMORY_CONTROLLER::ddr_get_channel(uint64_t address)
{
  int shift = LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(DDR_CHANNELS));
}

uint32_t MEMORY_CONTROLLER::ddr_get_bank(uint64_t address)
{
  int shift = lg2(DDR_CHANNELS) + LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(DDR_BANKS));
}

uint32_t MEMORY_CONTROLLER::ddr_get_column(uint64_t address)
{
  int shift = lg2(DDR_BANKS) + lg2(DDR_CHANNELS) + LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(DDR_COLUMNS));
}

uint32_t MEMORY_CONTROLLER::ddr_get_rank(uint64_t address)
{
  int shift = lg2(DDR_BANKS) + lg2(DDR_COLUMNS) + lg2(DDR_CHANNELS) + LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(DDR_RANKS));
}

uint32_t MEMORY_CONTROLLER::ddr_get_row(uint64_t address)
{
  int shift = lg2(DDR_RANKS) + lg2(DDR_BANKS) + lg2(DDR_COLUMNS) + lg2(DDR_CHANNELS) + LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(DDR_ROWS));
}

uint32_t MEMORY_CONTROLLER::get_occupancy(uint8_t queue_type, uint64_t address)
{
  uint32_t channel = memory_get_channel(address);
  MemoryType type = get_memory_type(address);
  ...
  case HBM:
    if (queue_type == 1)
      return std::count_if(std::begin(hbm_channels[channel].RQ), std::end(hbm_channels[channel].RQ), is_valid<PACKET>());
    else if (queue_type == 2)
      return std::count_if(std::begin(hbm_channels[channel].WQ), std::end(hbm_channels[channel].WQ), is_valid<PACKET>());
    else if (queue_type == 3)
      return get_occupancy(1, address);
    break;
  case DDR:
    if (queue_type == 1)
      return std::count_if(std::begin(ddr_channels[channel].RQ), std::end(ddr_channels[channel].RQ), is_valid<PACKET>());
    else if (queue_type == 2)
      return std::count_if(std::begin(ddr_channels[channel].WQ), std::end(ddr_channels[channel].WQ), is_valid<PACKET>());
    else if (queue_type == 3)
      return get_occupancy(1, address);
    break;
  }
...
uint32_t MEMORY_CONTROLLER::get_size(uint8_t queue_type, uint64_t address)
{
  ...
  case HBM:
    if (queue_type == 1)
      return hbm_channels[channel].RQ.size();
    else if (queue_type == 2)
      return hbm_channels[channel].WQ.size();
    else if (queue_type == 3)
      return get_size(1, address);
    break;
  case DDR:
    if (queue_type == 1)
      return ddr_channels[channel].RQ.size();
    else if (queue_type == 2)
      return ddr_channels[channel].WQ.size();
    else if (queue_type == 3)
      return get_size(1, address);
    break;
  }
...
MemoryType MEMORY_CONTROLLER::get_memory_type(uint64_t address)
{
  ...
  else if (address < HBM_CAPACITY + DDR_CAPACITY)
  {
    return DDR;
  }
  else
...
void MEMORY_CONTROLLER::operate_hbm(HBM_CHANNEL& channel)
{
  ...
  // For example, there are too many write requestes or some write requestes but no read requestes when in read mode,
  // or, there are no write request or some read requestes with little write requestes in write mode
  ...
  {
    uint32_t op_bank = hbm_get_bank(iter_next_schedule->address),
      op_row = hbm_get_row(iter_next_schedule->address);
    ...
    if (!channel.bank_request[op_idx].valid)
    {
      ...
      statistics.total_issued_memory_request_number++;
      statistics.memory_request_total_service_time += channel.bank_request[op_idx].event_cycle - iter_next_schedule->event_cycle;
...
void MEMORY_CONTROLLER::operate_ddr(DDR_CHANNEL& channel)
{
  // Finish request
  if (channel.active_request != std::end(channel.bank_request) && channel.active_request->event_cycle <= current_cycle)
  {
  ...
  if (is_valid<PACKET>()(*iter_next_schedule) && iter_next_schedule->event_cycle <= current_cycle)
  {
    uint32_t op_rank = ddr_get_rank(iter_next_schedule->address), op_bank = ddr_get_bank(iter_next_schedule->address),
      op_row = ddr_get_row(iter_next_schedule->address);

    auto op_idx = op_rank * DDR_BANKS + op_bank;
