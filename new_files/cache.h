...
public:
  ...
  enum class pref_t{pprefetcherDno, CPU_REDIRECT_pprefetcherDno_instr_};

  void pref_pprefetcherDno_initialize();
  void impl_prefetcher_initialize()
  {
    if (pref_type == pref_t::CPU_REDIRECT_pprefetcherDno_instr_)
      return ooo_cpu[cpu]->pref_pprefetcherDno_instr_initialize();
    if (pref_type == pref_t::pprefetcherDno)
      return pref_pprefetcherDno_initialize();
